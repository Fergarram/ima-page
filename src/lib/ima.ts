...
export type TagArgs = [] | [Props] | [Child, ...Child[]] | [Props, ...Child[]];

export type TagFunction = (...args: TagArgs) => Element;
...
export function useStaticTags(): StaticTagProxy {
	return new Proxy({}, { get: staticTagGenerator }) as StaticTagProxy;
}

export function useTags(options?: string | TagOptions): TagProxy | StaticTagProxy {
	const is_static = typeof window === "undefined";

	// Handle backward compatibility - if options is a string, treat it as namespace
	const resolved_options: TagOptions =
...
export function tagGenerator(_: any, tag: string, options?: TagOptions): TagFunction {
	return (...args: any[]): Element => {
...
// Reactive nodes
const reactive_markers: (Comment | null | undefined)[] = [];
const reactive_callbacks: ((() => any) | null | undefined)[] = [];
const reactive_prev_values: (Node | null | undefined)[] = [];
let reactive_node_count = 0;

// Reactive attributes
const reactive_attr_elements: (Element | null | undefined)[] = [];
const reactive_attr_names: (string | null | undefined)[] = [];
const reactive_attr_callbacks: ((() => any) | null | undefined)[] = [];
...
function updateReactiveComponents(): void {
...
function cleanupDisconnectedReactives(): void {
...
function setupReactiveNode(callback: () => any, doc: Document): DocumentFragment {
...
function setupReactiveAttr(element: Element, attr_name: string, callback: () => any): void {
...
export function buildAttributesHtml(props: Props): string {
	...
	for (const [key, value] of Object.entries(props)) {
		...
		// For non-event function values, call them to get the initial value
		const resolved_value = typeof value === "function" ? value() : value;

		// Regular attributes
		if (resolved_value === true) {
			html += ` ${key}`;
		} else if (resolved_value !== false && resolved_value != null) {
			html += ` ${key}="${escapeHtml(String(resolved_value))}"`;
		}
	}
...
function staticTagGenerator(_: any, tag: string): StaticTagFunction {
